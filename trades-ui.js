(() => {
  ...
  function injectSimulationStyles() {
    ...
    style.textContent = `
      ...
      .trade-sim-result .k{font-size:11px;color:var(--muted)}
      .trade-sim-profit{font-size:28px;font-weight:800;margin-top:4px}
      .trade-sim-profit.good{color:var(--good)}
      .trade-sim-profit.bad{color:var(--bad)}
  ...
  function setTradeModalTitle(title) {
    const modal = $('tradeVisualModal');
    const h2 = modal?.querySelector('.section-head h2');
  ...
  function prepareForm() {
    ...
    $('tradeVisualModal').dataset.mode = 'new';
    $('tradeVisualModal').dataset.tradeId = '';
    setTradeModalTitle('Novo trade');
    updateForm();
  }
  function updateForm() {
    const a = currentArb();
  ...
  async function submitForm(e) {
    ...
    try {
      const saved = await api(id ? `/rest/v1/trades?id=eq.${encodeURIComponent(id)}&user_id=eq.${encodeURIComponent(session.user.id)}` : '/rest/v1/trades', { method: id ? 'PATCH' : 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify(payload) });
      if (!id && saved?.[0]) {
  ...
  async function fetchLivePrices(symbols) {
    ...
    const query = `?ids=${encodeURIComponent(ids.join(','))}&vs_currencies=usd`;
    const endpoint = `${String(cfg.url || '').replace(/\/$/, '')}${LIVE_FN}${query}`;
    if (!cfg.url || !cfg.anonKey) throw new Error('Serviço de cotação não configurado.');
    let res;
    try {
      res = await fetch(endpoint, { cache: 'no-store', headers: { apikey: cfg.anonKey, Accept: 'application/json' } });
    } catch (e) {
      throw new Error(`Falha de conexão com o serviço de cotação: ${e.message || 'Failed to fetch'}`);
    }
    if (!res.ok) {
      let detail = '';
      try { const body = await res.json(); detail = body?.error ? `: ${body.error}` : ''; } catch {}
      if (res.status === 429) throw new Error('Limite temporário da consulta de cotação. Tente novamente em alguns segundos.');
      if (res.status === 401) throw new Error('Serviço de cotação recusou a chave pública do Supabase.');
      throw new Error(`Serviço de cotação indisponível (HTTP ${res.status})${detail}.`);
    }
    const payload = await res.json();
    const out = {};
    normalized.forEach((symbol, i) => {
      const price = Number(payload?.[ids[i]]?.usd);
  ...
  async function refreshSimulationQuote() {
    ...
    } catch (e) { renderSimulationError(e.message || 'Erro desconhecido.'); }
    finally { $('tradeSimRefresh').disabled = false; $('tradeSimRefresh').textContent = '⚡ Cotação agora'; }
  }
  ...
  async function renderTrades() {
    ...
    try {
      ...
      const rows = await api(`/rest/v1/trades?select=*&arbitrage_id=eq.${encodeURIComponent(arb.id)}&user_id=eq.${uid}&order=opened_at.desc`);
      ...
      open.textContent = `Erro ao carregar trades: ${message}`;
      closed.textContent = '';
    }
  }
  ...
  function showForm(t, closeMode = false) {
    ...
    $('tradeVisualStrategy').innerHTML = strategies(a).map(x => `<option value="${x}" ${x === t.strategy ? 'selected' : ''}>${x}</option>`).join('');
    ...
    modal.dataset.mode = closeMode ? 'close' : 'edit';
    setTradeModalTitle(closeMode ? 'Fechar trade' : 'Editar');
    updateForm();
  ...
  async function deleteTrade(t) {
    ...
    try {
      await api(`/rest/v1/trades?id=eq.${encodeURIComponent(t.id)}&user_id=eq.${encodeURIComponent(session.user.id)}`, { method: 'DELETE' });
      await renderTrades();
