(() => {
  ...
  function renderSimulationError(message) { $('tradeSimBody').innerHTML = `<div class="trade-sim-result"><strong>Não foi possível simular agora.</strong><div class="trade-sim-meta">${message}</div></div>`; }
  function renderSimulation(t, prices) { const anchor = String(t.anchor_symbol || '').toUpperCase(); const asset = String(t.current_asset || '').toUpperCase(); const initial = Number(t.initial_anchor_amount); const qty = Number(t.current_quantity); const assetPrice = Number(prices[asset]); const anchorPrice = Number(prices[anchor]); const simulatedClose = qty * assetPrice / anchorPrice; const profit = simulatedClose - initial; const pct = initial > 0 ? (profit / initial) * 100 : NaN; const entry = Number(t.entry_ratio_anchor_per_asset); const currentRatio = assetPrice > 0 && anchorPrice > 0 ? assetPrice / anchorPrice : NaN; const captured = new Date().toLocaleString('pt-BR'); const resultClass = profit >= 0 ? 'good' : 'bad'; $('tradeSimSubtitle').textContent = `${t.strategy} · cotação capturada em ${captured}`; $('tradeSimBody').innerHTML = `<div class="trade-sim-grid"><div class="trade-sim-card"><div class="k">Posição atual</div><div class="v">${fmt(qty)} ${asset}</div></div><div class="trade-sim-card"><div class="k">Capital inicial</div><div class="v">${fmt(initial)} ${anchor}</div></div><div class="trade-sim-card"><div class="k">${asset} agora</div><div class="v">${fmt(assetPrice, 10)} USD</div></div><div class="trade-sim-card"><div class="k">${anchor} agora</div><div class="v">${fmt(anchorPrice, 10)} USD</div></div><div class="trade-sim-card"><div class="k">Entrada registrada</div><div class="v">${fmt(entry, 10)} ${anchor}/${asset}</div></div><div class="trade-sim-card"><div class="k">Relação atual</div><div class="v">${fmt(currentRatio, 10)} ${anchor}/${asset}</div></div></div><div class="trade-sim-result"><div class="k">Fechamento simulado</div><div class="trade-sim-profit ${resultClass}">${fmt(simulatedClose)} ${anchor}</div><div class="trade-sim-meta">Resultado potencial: <strong>${fmt(profit)} ${anchor}</strong> · <strong>${Number.isFinite(pct) ? pct.toLocaleString('pt-BR',{maximumFractionDigits:4}) : '—'}%</strong></div><div class="trade-sim-meta">Cálculo: ${fmt(qty)} ${asset} × ${fmt(assetPrice, 10)} USD ÷ ${fmt(anchorPrice, 10)} USD = ${fmt(simulatedClose)} ${anchor}.</div><div class="trade-sim-meta">Simulação não grava o trade e não considera taxas, slippage, spread ou impacto de mercado.</div></div>`; }
