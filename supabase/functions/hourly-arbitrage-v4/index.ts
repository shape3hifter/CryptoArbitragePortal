...
function formatMessage(e: any) {
  ...
  if (e.type === "STATE_CHANGE_HOLD_TO_BUY") return `🟢 **ARBITRAGEM — BUY**\n\n**${name}**${strategy}${metrics}${e.has_open_trade ? "\n💼 **Trade aberto**" : "\n⚠️ **Não há trade aberto."}`;
  if (e.type === "STATE_CHANGE_HOLD_TO_SELL") {
    if (e.has_open_trade) return `💼 **TRADE ABERTO**\n\n**${name}**\n${e.strategy ?? ""}\n\n🔴 **SELL**\n\n**FECHAMENTO SIMULADO**\n${numfmt(e.current_anchor_amount)} ${e.anchor_symbol}\n\n**RESULTADO POTENCIAL**\n${numfmt(e.profit_anchor_amount)} ${e.anchor_symbol} · **${signedPct(e.profit_pct)}**\n\nCapital inicial\n${numfmt(e.initial_anchor_amount)} ${e.anchor_symbol}\n\n📊 Z-Score: **${zfmt(e.zscore)}**\n📈 GAP: **${signedPct(e.gap_pct)}**`;
  ...
  if (String(e.type).startsWith("BUY_THRESHOLD_")) {
    const l = Number(e.threshold) * 100, t = e.has_open_trade ? `\n\n💼 **Trade aberto**\nResultado atual: **${signedPct(e.profit_pct)}**` : `\n\n⚠️ **Não há trade aberto.**`;
    return `🟢 **ARBITRAGEM — BUY - ${l.toFixed(0)}%**\n\n**${name}**${strategy}${metrics}\n\n📈 GAP atingiu **${l.toFixed(0)}%**${t}`;
  }
  if (String(e.type).startsWith("PROFIT_THRESHOLD_")) {
    const l = Number(e.threshold) * 100;
    return `💰 **TRADE — LUCRO +${l.toFixed(0)}%**\n\n**${name}**${strategy}\n\n💵 Resultado atual: **${signedPct(e.profit_pct)}**\n\n📌 Nível de lucro atingido: **${l.toFixed(0)}%**\n\n💼 Trade permanece aberto.`;
  }
  return null;
}
...
function htmlFor(text: string) {
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.5;color:#111827;white-space:pre-wrap;">${esc.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>").replace(/\n/g, "<br>")}</div>`;
}
function subjectFor(t: string) { if (t.startsWith("PROFIT_THRESHOLD_")) return "Crypto Arbitrage — Lucro atingido"; if (t.startsWith("BUY_THRESHOLD_")) return "Crypto Arbitrage — BUY"; if (t.includes("SELL")) return "Crypto Arbitrage — SELL"; if (t.includes("BUY_TO_HOLD")) return "Crypto Arbitrage — BUY → HOLD"; if (t.includes("SELL_TO_HOLD")) return "Crypto Arbitrage — SELL → HOLD"; return "Crypto Arbitrage — Alerta"; }
async function sendEmail(row: any) { if (!RESEND_KEY) throw Error("RESEND_API_KEY is not configured"); const payload = { from: EMAIL_FROM, to: [EMAIL_TO], subject: subjectFor(row.event_type), text: row.message, html: htmlFor(row.message) }; const r = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${RESEND_KEY}`, "Content-Type": "application/json", "Idempotency-Key": `arbitrage/${row.id}` }, body: JSON.stringify(payload) }); const b = await r.json().catch(() => ({})); if (!r.ok) throw Error(`Resend ${r.status}: ${b?.message ?? JSON.stringify(b)}`); return String(b?.id ?? ""); }
  ...
  try {
    ...
    for (const uid of users) for (const arbId of Object.keys(ARBS)) {
      ...
      for (const asset of c.comparatives) {
        ...
        const r = rs.at(-1), w = rs.slice(-LOOKBACK), avg = mean(w), stdev = sd(w), zs = stdev === 0 ? 0 : (r - avg) / stdev, g = avg === 0 ? null : r / avg - 1, s = signal(zs), k = `${uid}|${arbId}|${asset}`, prev = sm.get(k), ps = prev?.signal ?? "HOLD";
        const trade = targetTrades.find((t: any) => t.user_id === uid && t.arbitrage_id === arbId && String(t.current_asset).toUpperCase() === asset);
        let tradeCurrent = null, tradeProfit = null, initial = null;
        if (trade) { const tr = ratio(prices, trade.current_asset, trade.anchor_symbol); initial = Number(trade.initial_anchor_amount); if (tr !== null && Number.isFinite(initial) && initial !== 0) { tradeCurrent = Number(trade.current_quantity) * tr; tradeProfit = tradeCurrent / initial - 1; } }
        const base = { user_id: uid, arbitrage_id: arbId, arbitrage_name: c.name, comparative_symbol: asset, anchor_symbol: c.anchor, strategy: c.strategies[asset], signal: s, previous_signal: ps, gap_pct: g, zscore: zs, has_open_trade: !!trade, evaluated_at: at, current_anchor_amount: tradeCurrent, profit_anchor_amount: tradeCurrent !== null && initial !== null ? tradeCurrent - initial : null, initial_anchor_amount: initial, profit_pct: tradeProfit };
        let th = prev?.gap_threshold_alerted ?? null, cy = prev?.cycle_started_at ?? null;
        if (s !== ps) { const type = `STATE_CHANGE_${ps}_TO_${s}`; events.push({ ...base, type }); const msg = formatMessage({ ...base, type }); if (msg) previews.push({ ...base, type, trade_id: trade?.id ?? null, profit_pct: tradeProfit, threshold: null, message: msg }); }
        if (s === "BUY") { if (ps !== "BUY") { th = null; cy = at; } const lv = level(Math.abs(g ?? 0), BUY, th); if (lv !== null) { th = lv; const type = `BUY_THRESHOLD_${Math.round(lv * 100)}`, msg = formatMessage({ ...base, type, threshold: lv }); events.push({ ...base, type, threshold: lv }); if (msg) previews.push({ ...base, type, trade_id: trade?.id ?? null, profit_pct: tradeProfit, threshold: lv, message: msg }); } } else { th = null; if (s === "SELL" && ps !== "SELL") cy = at; if (s === "HOLD" && ps !== "HOLD") cy = null; }
        ...
        su.push({ user_id: uid, arbitrage_id: arbId, comparative_symbol: asset, signal: s, gap_pct: s === "BUY" ? g : null, zscore: zs, gap_threshold_alerted: th, cycle_started_at: cy, evaluated_at: at, updated_at: at });
      }
    }
    for (const t of targetTrades) {
      ...
      const current = Number(t.current_quantity) * r, profit = current / initial - 1, prev = tm.get(`${t.user_id}|${t.id}`), old = prev?.profit_threshold_alerted ?? null, lv = level(Math.max(0, profit), PROFIT, old); let alerted = old;
      if (lv !== null) { alerted = lv; const type = `PROFIT_THRESHOLD_${Math.round(lv * 100)}`, c = ARBS[t.arbitrage_id], base = { user_id: t.user_id, trade_id: t.id, arbitrage_id: t.arbitrage_id, arbitrage_name: t.arbitrage_name || c?.name || "ARBITRAGEM", comparative_symbol: t.current_asset, anchor_symbol: t.anchor_symbol, strategy: c?.strategies?.[t.current_asset], signal: null, gap_pct: null, zscore: null, has_open_trade: true, evaluated_at: at, current_anchor_amount: current, profit_anchor_amount: current - initial, initial_anchor_amount: initial, profit_pct: profit, type, threshold: lv }; events.push(base); const msg = formatMessage(base); if (msg) previews.push({ ...base, message: msg }); }
